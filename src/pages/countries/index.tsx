...
import { getCountries, deleteCountryById } from 'apiSdk/countries';
import { CountryInterface } from 'interfaces/country';

type ColumnType = ColumnDef<CountryInterface, unknown>;
...
export function CountryListPage(props: CountryListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CountryInterface>>(
    () => `/countries?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: CountryInterface) => {
    if (hasAccess('country', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/countries/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/countries/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('country', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/countries/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('country', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="country" />
        </Flex>

        {hasAccess('country', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/countries/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
